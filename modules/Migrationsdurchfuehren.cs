...
{
    public class Migrationsdurchfuehren
    {

        public static string DbMigrieren(string as_versionVon, string as_versionBis, string as_kunde)
        {
            ...
            #region Miragtionsordner kopieren

             string migrationsOrdner = Data.GrossMigration5 + Data.GrossMigration1 + las_VersionVon[0] + "." + las_VersionVon[1] + "\\" + Data.GrossMigration2 + as_versionBis;

            
            string zielPfad = Data.lokalMigDurchfuehren + as_versionBis;
            if(!Directory.Exists(zielPfad))
            {
                CopyMigrationDirectory(migrationsOrdner, zielPfad);
            }
            ...
            string client_id = Data.GetKundenDatabase(as_kunde).ToLower();
            string client_pw = Data.GetKundenDatabase(as_kunde).ToLower();
            string client_server = Data.GetKundenServer(as_kunde);
        ...
        private static void CopyMigrationDirectory(string sourceDir, string targetDir)
        {
        ...
        private static string SteuerDateiErstellen(string targetDir, string versionVon, string versionBis, string kunde)
        {
            ...
            string[] las_VersionBis = versionBis.Split('.');
                ...
                string kundenDatenbank = Data.GetKundenDatabase(kunde).ToLower();

                string kundenserver = Data.GetKundenServer(kunde).ToLower();

                if (kundenDatenbank != "" || kundenserver !="")
                {
